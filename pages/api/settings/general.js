  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      const db = connectToDatabase();
        ...
        const existingDocument = await db.collection('general_settings').get();

        // Update or insert based on the existence of the document
        if (existingDocument.docs.length > 0) {
            await db.collection('general_settings').doc(existingDocument.docs[0].id).update(generalSettingsDocument);
        } else {
            await db.collection('general_settings').add(generalSettingsDocument);
        }
